...
export const WatchProvidersDialog = ({
    ...
    const hasWatchProviders = media.watch_providers?.results?.IT &&
        (media.watch_providers.results.IT.flatrate?.length > 0 ||
            media.watch_providers.results.IT.rent?.length > 0 ||
            media.watch_providers.results.IT.buy?.length > 0);
            ...
            <DialogContent className="sm:max-w-md">
                ...
                {hasWatchProviders ? (
                    <div className="space-y-4 max-h-[60vh] overflow-y-auto py-4">
                        {media.watch_providers?.results?.IT?.flatrate && (
                            <div>
                                <h3 className="font-medium mb-2 text-accent">Streaming</h3>
                                <div className="flex flex-wrap gap-4">
                                    {media.watch_providers.results.IT.flatrate.map(provider => (
                                        <div
                        ...
                        {media.watch_providers?.results?.IT?.rent && (
                            <div>
                                <h3 className="font-medium mb-2 text-accent">Noleggio</h3>
                                <div className="flex flex-wrap gap-4">
                                    {media.watch_providers.results.IT.rent.map(provider => (
                                        <div
                                            key={provider.provider_id}
                                            className="flex flex-col items-center gap-1"
                                        >
                        ...
                        {media.watch_providers?.results?.IT?.buy && (
                            <div>
                                <h3 className="font-medium mb-2 text-accent">Acquisto</h3>
                                <div className="flex flex-wrap gap-4">
                                    {media.watch_providers.results.IT.buy.map(provider => (
                                        <div
                                            key={provider.provider_id}
                                            className="flex flex-col items-center gap-1"
                                        >
