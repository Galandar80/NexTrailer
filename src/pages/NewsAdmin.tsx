  ...
  const loadArticles = useCallback(async () => {
    ...
    try {
      ...
      const newsFetched = newsSnapshot.docs.map((entry) => {
        ...
        return { ...data, id: data.id || entry.id, collection: "news_articles" };
      });
      const topFetched = topSnapshot.docs.map((entry) => {
        const data = entry.data() as NewsArticle;
        return { ...data, id: data.id || entry.id, collection: "news_comingsoon" };
      });
        ...
        <DialogContent className="max-h-[90vh] w-[calc(100%-1.5rem)] overflow-y-auto sm:max-w-3xl">
          ...
          <div className="space-y-4">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-3 grid gap-2 sm:grid-cols-2">
                          ...
                          >
                            {posterUrl ? (
                              <img src={posterUrl} alt={`${title} poster`} className="h-16 w-12 rounded-md object-cover" />
                            ) : (
                          ...
                          >
                            {backdropUrl ? (
                              <img src={backdropUrl} alt={`${title} backdrop`} className="h-16 w-24 rounded-md object-cover" />
                            ) : (
            ...
            <div className="space-y-2">
              ...
              />
              {editImageUrl && (
                <img src={editImageUrl} alt={editTitle || "Immagine articolo"} className="w-full max-w-md rounded-lg border border-border" />
              )}
            </div>
