...
const Movies = () => {
  ...
  const [movieArticles, setMovieArticles] = useState<Article[]>([]);
  ...
  useEffect(() => {
    const loadContent = async () => {
      ...
      try {
        const [popular, topRated, upcoming, articles] = await Promise.all([
          tmdbApi.getPopular("movie"),
          tmdbApi.getTopRated("movie"),
          tmdbApi.getUpcoming(),
          tmdbApi.getArticles("movie", 6)
        ]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="max-w-screen-2xl mx-auto px-4 md:px-8 py-6">
        <h1 className="text-3xl font-poster mb-8">Film</h1>
          ...
          <>
            <section className="mb-12">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="text-2xl font-medium">Novità dal Cinema</h2>
              </div>
